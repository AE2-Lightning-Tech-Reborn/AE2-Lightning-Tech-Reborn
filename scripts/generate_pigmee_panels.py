...
def generate():
    ...
    for color, zh, hex_value in PALETTE:
        for framed in (False, True):
            ...
            write_json(ASSETS / f"blockstates/{name}.json", {
                "variants": {"": {"type": "ae2lt:connected_texture",
                    "base": texture_path, "ctm": texture_path + "_ctm", "connection": "ae2lt:same_block",
            ...
            write_json(ASSETS / f"items/{name}.json", {
                "model": {"type": "minecraft:model", "model": f"ae2lt:item/{name}"}})
    ...
    write_json(DATA / f"recipe/{restore_recipe}.json", {"type": "minecraft:crafting_shapeless",
        "category": "building", "ingredients": ["#ae2lt:pigmee_finished_building_panels"],
