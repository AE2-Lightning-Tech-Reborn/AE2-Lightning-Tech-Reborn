...
def unlock(recipe_name, ingredient):
    write_json(DATA / f"advancements/recipes/pigmee_building/{recipe_name}.json", {
        "parent": "minecraft:recipes/root",
        "criteria": {
            "has_material": {"trigger": "minecraft:inventory_changed", "conditions": {
                "items": [{"tag": ingredient[1:]} if ingredient.startswith("#") else {"items": [ingredient]}]}},
...
def stonecut(name, ingredient, output):
    write_json(DATA / f"recipes/{name}.json", {"type": "minecraft:stonecutting",
...
def generate():
    ...
    for color, zh, hex_value in PALETTE:
        for framed in (False, True):
            ...
            write_json(DATA / f"loot_tables/blocks/{name}.json", {"type": "minecraft:block",
    ...
    for framed in (False, True):
        ...
        for registry in ("item", "block"):
            write_json(DATA / f"tags/{registry}s/{tag}.json", {"replace": False, "values": ids})
    write_json(DATA / "tags/items/pigmee_building_materials.json", {"replace": False,
        "values": ["ae2lt:pigmee_building_block", "#ae2lt:pigmee_finished_building_panels"]})
    write_json(DATA / "tags/items/pigmee_finished_building_panels.json", {"replace": False,
        "values": ["#ae2lt:pigmee_building_panels", "#ae2lt:pigmee_framed_building_panels"]})
    restore_recipe = "pigmee_building_block_from_panels"
    write_json(DATA / f"recipes/{restore_recipe}.json", {"type": "minecraft:crafting_shapeless",
        "category": "building", "ingredients": [{"tag": "ae2lt:pigmee_finished_building_panels"}],
        "result": {"item": "ae2lt:pigmee_building_block", "count": 1}})
    ...
    for identifier in ["ae2lt:pigmee_building_block", *all_ids]:
