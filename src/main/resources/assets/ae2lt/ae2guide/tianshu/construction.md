...
The scene below shows one complete structure using a Tianshu Supercomputer Baseline Main Core. Drag to rotate the scene. The centers of the bottom and top faces are the two possible port positions.
...
An existing Port may occupy either candidate. If there is no Port, one candidate already contains a Tianshu Supercomputer Cooling Unit or closed-loop storage, and the other is empty, auto-build keeps the existing block and places the Port in the empty position. If both candidates are empty, the lower position is used by default. If both contain non-Port blocks, clear one position first.
...
* Exactly **one** of the two port candidates must contain a Tianshu Port; the other accepts a Tianshu Supercomputer Cooling Unit, Closed-Loop Pattern Storage, or Closed-Loop Seed Storage
...
* Baseline, Quantum, and Overload run without functional units; Storage and Parallel Units are optional
...
* All 16 fixed cooling positions and the unused port candidate require a Tianshu Supercomputer Cooling Unit or one of the two closed-loop storage blocks
