...
public class TianshuCraftingTermScreen<M extends TianshuCraftingTermMenu> extends TianshuMaintenanceTermScreen<M> {
    ...
    @Override protected void slotClicked(Slot slot, int index, int mouseButton, ContainerInput type) {
        if (menu.isWorkResult(slot)) {
            ...
            var action = net.minecraft.client.Minecraft.getInstance().hasShiftDown() ? InventoryAction.CRAFT_SHIFT
                    : InputConstants.isKeyDown(getMinecraft().getWindow(), GLFW.GLFW_KEY_SPACE)
                    ? InventoryAction.CRAFT_ALL : mouseButton == 1 ? InventoryAction.CRAFT_STACK : InventoryAction.CRAFT_ITEM;
            net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new InventoryActionPacket(action, index, 0));
    ...
    @Override protected void updateBeforeRender() {
        ...
        stoneScrollbar.setRange(0, (menu.getStonecutter().getNumberOfVisibleRecipes() + stoneColumns() - 1) / stoneColumns() - 2, 2);
        ...
        for (var button : stoneButtons) {
            button.visible = page == TianshuWorkPage.STONECUTTING && button.index < stoneColumns() * 2
                    && button.recipeIndex() < menu.getStonecutter().getNumberOfVisibleRecipes();
    ...
    @Override public boolean mouseClicked(net.minecraft.client.input.MouseButtonEvent event, boolean handled) {
        int button = event.button();
        if (handleMaintenanceClick(button)) return true;
        if (menu.workPage == TianshuWorkPage.CELL && net.minecraft.client.Minecraft.getInstance().hasControlDown()
    ...
    @Override public void drawBG(GuiGraphicsExtractor graphics, int offsetX, int offsetY, int mouseX, int mouseY, float partialTicks) {
        ...
        if (compact && menu.workPage == TianshuWorkPage.STONECUTTING) {
            ...
            graphics.pose().translate(offsetX + 183, offsetY + imageHeight - 139);
    ...
    protected void drawWorkAreaBackground(GuiGraphicsExtractor graphics, int offsetX, int offsetY) {
        drawWorkAreaFrame(graphics, offsetX + 7, offsetY + imageHeight - 166, 162, 68);
    }

    protected final void drawWorkAreaFrame(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
    ...
    private void drawWorkSlot(GuiGraphicsExtractor g, int x, int y) {
        Blitter.texture("guis/crafting.png").src(25, 92, 18, 18).dest(x - 1, y - 1).blit(g);
    }

    private void drawCellSlot(GuiGraphicsExtractor graphics, int x, int y, boolean enabled) {
        // Same optional-slot texture and disabled opacity used by AE2's cell workbench.
        LegacyAe2IconBlitter.of(Icon.SLOT_BACKGROUND).dest(x - 1, y - 1).opacity(enabled ? 1.0f : 0.2f).blit(graphics);
    }
    ...
    @Override public void drawFG(GuiGraphicsExtractor graphics, int offsetX, int offsetY, int mouseX, int mouseY) {
        ...
        if (menu.workPage == TianshuWorkPage.CELL && menu.getMaxCellConfigRow() > 0) {
            graphics.text(font, (menu.cellConfigRow + 1) + "–" + Math.min(menu.cellConfigRow + 3, menu.getCellConfigRows()), compact ? 126 : 56,
                    imageHeight - 108, 0xFF413f54, false);
        } else if (menu.workPage == TianshuWorkPage.ANVIL) {
            graphics.text(font, "+", compact ? 115 : 58, imageHeight - (compact ? 137 : 129), 0xFF413f54, false);
    ...
    @Override public boolean keyPressed(net.minecraft.client.input.KeyEvent event) {
        ...
        if (anvilName.visible && anvilName.active && anvilName.isFocused() && key != org.lwjgl.glfw.GLFW.GLFW_KEY_ESCAPE) {
            return anvilName.keyPressed(event) || anvilName.canConsumeInput();
        }
    ...
    @Override public boolean charTyped(net.minecraft.client.input.CharacterEvent event) {
        ...
        if (anvilName.visible && anvilName.active && anvilName.isFocused()) return anvilName.charTyped(event);
    ...
    private final class StoneRecipeButton extends Button {
        ...
        ItemStack result() {
            return menu.getStonecutter().getVisibleRecipes().entries().get(recipeIndex()).recipe().optionDisplay()
                    .resolveForFirstStack(net.minecraft.world.item.crafting.display.SlotDisplayContext.fromLevel(menu.getPlayer().level()));
        }
        @Override protected void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
            if (recipeIndex() < menu.getStonecutter().getNumberOfVisibleRecipes()) {
                ...
                graphics.item(result(), getX() + 2, y);
                graphics.itemDecorations(font, result(), getX() + 2, y);
            }
        }
    }
    ...
    private static final class WorkPageTabButton extends TabButton {
        ...
        @Override public void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
            ...
            LegacyAe2IconBlitter.of(background).dest(getX(), getY(), getWidth(), getHeight()).blit(graphics);
            int iconY = getY() + (getHeight() - 16) / 2;
            if (nativeIcon != null) {
                LegacyAe2IconBlitter.of(nativeIcon).dest(getX() + 1, iconY).blit(graphics);
            } else {
                graphics.pose().pushMatrix();
                graphics.nextStratum();
                graphics.item(itemIcon, getX() + 1, iconY);
                graphics.pose().popMatrix();
            }
        }
    }
