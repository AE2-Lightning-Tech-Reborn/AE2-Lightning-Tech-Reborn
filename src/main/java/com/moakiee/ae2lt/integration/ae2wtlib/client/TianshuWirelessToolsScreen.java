...
final class TianshuWirelessToolsScreen extends AESubScreen<TianshuWirelessCraftingTermMenu, TianshuEnhancedWirelessCraftingScreen> {
    ...
    TianshuWirelessToolsScreen(TianshuEnhancedWirelessCraftingScreen parent, WirelessPage page) {
        ...
        widgets.add("back", new TabButton(Icon.ARROW_LEFT, menu.getHost().getMainMenuIcon().getHoverName(), b -> returnToParent()));
    ...
    private void addSettings() {
        ...
        pickBlock.setSelected(stack.getOrCreateTag().getBoolean("pickBlock"));
        craftIfMissing.setSelected(stack.getOrCreateTag().getBoolean("craftIfMissing"));
        craftIfMissing.active = pickBlock.isSelected();
        restock.setSelected(stack.getOrCreateTag().getBoolean("restock"));
        var mode = de.mari_023.ae2wtlib.wct.magnet_card.MagnetHandler.getMagnetMode(stack);
        ...
        Runnable save = () -> {
            craftIfMissing.active = pickBlock.isSelected();
            features.setWirelessSettings(new TianshuEnhancedWirelessCraftingMenu.WirelessSettings(
