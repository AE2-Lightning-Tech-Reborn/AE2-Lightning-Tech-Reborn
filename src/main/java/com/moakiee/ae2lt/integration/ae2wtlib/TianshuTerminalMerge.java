...
public final class TianshuTerminalMerge {
    ...
    public static ItemStack merge(ItemStack target, ItemStack source, WTDefinition definition) {
        ...
        for (var entry : source.getComponentsPatch().entrySet()) {
            ...
            if (type == ModDataComponents.TIANSHU_WORKSTATIONS.get()
                    && incoming instanceof CustomData work && existing instanceof CustomData otherWork
                    && TianshuWorkstationStorage.containsItems(work.copyTag())
                    && TianshuWorkstationStorage.containsItems(otherWork.copyTag())) return ItemStack.EMPTY;
    ...
    private static boolean containsRealPatterns(CompoundTag tag) {
        return !tag.getListOrEmpty("blankPattern").isEmpty()
                || !tag.getListOrEmpty("encodedPattern").isEmpty();
    }
