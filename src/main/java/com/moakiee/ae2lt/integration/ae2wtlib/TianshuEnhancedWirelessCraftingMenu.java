...
public final class TianshuEnhancedWirelessCraftingMenu extends TianshuWirelessCraftingTermMenu {
    ...
    public TianshuEnhancedWirelessCraftingMenu(int id, Inventory inventory, TianshuWirelessCraftingTermMenuHost host) {
        ...
        addSlot(new ArmorSlot(inventory, ArmorSlot.Armor.HEAD) {
            @Override public boolean mayPlace(ItemStack stack) {
                return (stack.getItem() instanceof BlockItem && stack.get(net.minecraft.core.component.DataComponents.EQUIPPABLE) instanceof net.minecraft.world.item.equipment.Equippable equipment && equipment.slot() == net.minecraft.world.entity.EquipmentSlot.HEAD) || super.mayPlace(stack);
            }
        ...
        registerClientAction(new appeng.menu.guisync.ClientActionKey<WirelessPage>("wirelessPage"), net.minecraft.network.codec.StreamCodec.of((buf, value) -> buf.writeEnum(value), buf -> buf.readEnum(WirelessPage.class)), this::setWirelessPage);
        for (var action : TianshuMagnetMenu.ACTIONS) {
            registerClientAction(new appeng.menu.guisync.ClientActionKey<Void>(action), () -> {
                if (wirelessPage != WirelessPage.MAGNET || !hasMagnetCard()) return;
                magnetMenu.receiveClientAction(action, new byte[0]);
                invalidateFilters();
                broadcastChanges();
            });
        }
    ...
    public void setWirelessPage(WirelessPage page) {
        if (isClientSide()) { wirelessPage = page; sendClientAction(new appeng.menu.guisync.ClientActionKey<>("wirelessPage"), page); return; }
