...
public final class InteractiveMultiblockPreview {
    ...
    private final SlotDelegate slots;
    ...
    public InteractiveMultiblockPreview(
    ...
    private void drawToolbar(GuiGraphicsExtractor guiGraphics, Font font, double mouseX, double mouseY) {
        drawControlButton(
                guiGraphics,
                resetButton,
                MultiblockPreviewControls.PixelIcon.RESET_VIEW,
                false,
                true,
                mouseX,
                mouseY);
        drawControlButton(
                guiGraphics,
                autoButton,
                autoRotate ? MultiblockPreviewControls.PixelIcon.PAUSE : MultiblockPreviewControls.PixelIcon.PLAY,
                autoRotate,
                true,
                mouseX,
                mouseY);
        drawControlButton(
                ...
                shellButton,
                hideShell
                        ? MultiblockPreviewControls.PixelIcon.SHELL_HIDDEN
                        : MultiblockPreviewControls.PixelIcon.SHELL,
                hideShell,
                true,
                mouseX,
                mouseY);
        ...
        drawControlButton(
                guiGraphics,
                layerDownButton,
                MultiblockPreviewControls.PixelIcon.MINUS,
                false,
                layer > 0,
                mouseX,
                mouseY);
        drawControlButton(
                guiGraphics,
                layerUpButton,
                MultiblockPreviewControls.PixelIcon.PLUS,
                false,
        ...
        Component text = Component.translatable(
                ...
                layer + 1,
                recipe.sizeY());
        MultiblockPreviewControls.drawInsetLabel(
                guiGraphics,
                font,
                layerLabel.x(),
                layerLabel.y(),
    ...
    private void drawMaterials(GuiGraphicsExtractor guiGraphics, Font font, double mouseX, double mouseY) {
        ...
        int count = Math.min(materials.size(), slots.materialSlotCount());
        for (int i = 0; i < count; i++) {
            ...
            slots.drawMaterialSlot(guiGraphics, i, panelX + 4, rowY + 4);
    ...
    private void drawDetails(GuiGraphicsExtractor guiGraphics, Font font) {
        ...
        slots.drawSelectedBlockSlot(guiGraphics, selectedBlock, panelX + 4, contentTop + 2);
        ...
        if (alternatives.size() == 1 && !allowsAir) {
            ...
            for (int i = 0; i < Math.min(alternatives.size(), slots.alternativeSlotCount()); i++) {
                int x = panelX + 4 + i % 4 * 23;
                int y = contentTop + 63 + i / 4 * 21;
                slots.drawAlternativeSlot(guiGraphics, i, alternatives.get(i), x, y);
            }
            if (allowsAir) {
    ...
    public Optional<SlotReference> getSlotUnderMouse(double mouseX, double mouseY) {
        if (panelTab == PanelTab.MATERIALS) {
            if (!inside(panelX, panelContentTop(), PANEL_WIDTH, panelContentHeight(), mouseX, mouseY)) {
                return Optional.empty();
            }
            for (int i = 0; i < Math.min(recipe.materials().size(), slots.materialSlotCount()); i++) {
                int rowY = materialRowY(i);
                if (rowY + MATERIAL_ROW_HEIGHT <= panelContentTop() || rowY >= panelContentBottom()) {
                    continue;
                }
                if (inside(panelX + 4, rowY + 4, 18, 18, mouseX, mouseY)) {
                    return Optional.of(new SlotReference(SlotKind.MATERIAL, i));
                }
            }
            return Optional.empty();
        }
        ...
        if (inside(panelX + 4, panelContentTop() + 2, 18, 18, mouseX, mouseY)) {
            return Optional.of(new SlotReference(SlotKind.SELECTED, 0));
        }
        int visibleCount = Math.min(visibleAlternatives().size(), slots.alternativeSlotCount());
        for (int i = 0; i < visibleCount; i++) {
            int x = panelX + 4 + i % 4 * 23;
            int y = panelContentTop() + 63 + i / 4 * 21;
            if (inside(x, y, 18, 18, mouseX, mouseY)) {
                return Optional.of(new SlotReference(SlotKind.ALTERNATIVE, i));
            }
        }
        return Optional.empty();
    }
    ...
    public record SlotReference(SlotKind kind, int index) {
    }
