...
public final class TianshuCraftingTransferHandler<M extends TianshuCraftingTermMenu> implements IUniversalRecipeTransferHandler<M> {
    ...
    @Nullable @Override public IRecipeTransferError transferRecipe(M menu, Object displayedRecipe, IRecipeSlotsView slots, Player player, boolean maxTransfer, boolean doTransfer) {
        ...
        if (displayedRecipe instanceof IJeiAnvilRecipe anvil) {
            ...
            var left = inputs.isEmpty() ? ItemStack.EMPTY : inputs.get(0).getDisplayedItemStack().orElse(ItemStack.EMPTY);
        ...
        if (!(displayedRecipe instanceof Recipe<?> recipe)) return helper.createInternalError();
        if (recipe instanceof CraftingRecipe crafting) {
            if (doTransfer) menu.prepareCraftingTransfer();
            return new UseCraftingRecipeTransfer<M>(menuClass, menuType, helper)
                    .transferRecipe(menu, crafting, slots, player, maxTransfer, doTransfer);
        }
        if (!(recipe instanceof SmithingRecipe || recipe instanceof StonecutterRecipe)) return helper.createInternalError();
        boolean craftMissing = AbstractContainerScreen.hasControlDown();
        var error = workFeedback(menu.getWorkRecipeAvailability(recipe), slots, craftMissing, doTransfer);
        if (error == null && doTransfer) menu.fillWorkRecipe(recipe.getId().toString(), craftMissing);
        return error;
    }
    ...
    @Nullable private IRecipeTransferError workFeedback(TianshuCraftingTermMenu.WorkRecipeAvailability available,
        ...
        return !doTransfer && missing.totalSize() > 0 ? new CraftingFeedback(missing, craftMissing) : null;
    }
    ...
    private record CraftingFeedback(CraftingTermMenu.MissingIngredientSlots ingredients, boolean craftMissing)
        ...
        @Override public void getTooltip(ITooltipBuilder tooltip) {
            tooltip.addAll(TransferHelper.createCraftingTooltip(ingredients, craftMissing));
        }
