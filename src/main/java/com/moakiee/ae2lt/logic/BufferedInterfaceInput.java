...
public final class BufferedInterfaceInput {
    ...
    public ListTag write(HolderLookup.Provider registries) {
        ...
        entries.forEach((key, amount) -> result.add(
                com.moakiee.ae2lt.recipe.compat.LegacyAeStackTags.writeGeneric(registries, new GenericStack(key, amount))));
        return result;
    }

    public void read(ListTag saved, HolderLookup.Provider registries) {
        clear();
        for (int i = 0; i < saved.size(); i++) {
            var stack = com.moakiee.ae2lt.recipe.compat.LegacyAeStackTags.readGeneric(registries, saved.getCompoundOrEmpty(i));
    ...
    /** Remaining shared capacity while a NeoForge transaction is staging inputs. */
    public long capacityAfterPending(AEKey key, long requested, java.util.List<com.moakiee.ae2lt.recipe.compat.ActionableInventoryTransfer.Operation> pending) {
        long reserved = 0;
        var newKeys = new java.util.HashSet<AEKey>();
        for (var operation : pending) {
            if (!operation.insertion()) continue;
            if (operation.key().getType() == key.getType()) reserved += operation.amount();
            if (!entries.containsKey(operation.key())) newKeys.add(operation.key());
        }
        if (!entries.containsKey(key) && !newKeys.contains(key) && entries.size() + newKeys.size() >= MAX_KEYS) return 0;
        return Math.min(requested, Math.max(0, capacity(key.getType()) - typeAmounts.getOrDefault(key.getType(), 0L) - reserved));
    }
