...
public record CountedIngredient(Ingredient ingredient, int count) {
    ...
    public static final MapCodec<CountedIngredient> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
                    Ingredient.CODEC.validate(ingredient -> ingredient.isEmpty()
                            ? com.mojang.serialization.DataResult.error(() -> "ingredient cannot be empty")
                            : com.mojang.serialization.DataResult.success(ingredient)).fieldOf("ingredient").forGetter(CountedIngredient::ingredient),
