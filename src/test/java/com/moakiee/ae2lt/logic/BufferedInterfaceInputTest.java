...
import com.moakiee.ae2lt.recipe.compat.ActionableInventoryTransfer;
...
class BufferedInterfaceInputTest extends com.moakiee.ae2lt.test.MinecraftComponentsTestBase {
    @BeforeAll
    ...
    void repeatedRealItemHandlerSimulationsAndInsertionsCoalesceBeforeNetworkAccess() {
        ...
        var direct = new LegacyItemHandlerView(new ActionableInventoryTransfer(delegate, null).items());
        var buffered = new LegacyItemHandlerView(new ActionableInventoryTransfer(new FilteredInsertGenericInv(
                delegate, key -> !key.equals(AEItemKey.of(Items.DIRT)),
                (slot, key, amount, mode) -> queue.insert(key, amount, mode)),
                (slot, key, amount, insert, pending) -> queue.capacityAfterPending(key, amount, pending)).items());
        ...
        assertEquals(30_000, network.calls);
        network.calls = 0;
        ...
        System.out.println("passive-input comparison: 10000 simulate+insert pairs; network calls 30000 -> 1; conserved=10000");
    }
