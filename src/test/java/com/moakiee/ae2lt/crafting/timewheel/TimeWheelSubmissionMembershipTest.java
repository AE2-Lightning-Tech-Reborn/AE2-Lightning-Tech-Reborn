...
class TimeWheelSubmissionMembershipTest {
    ...
    void cachedSelectionMustStillBelongToTheSubmittingGrid(
        ...
        var types = Class.forName("appeng.api.stacks.AEKeyTypesInternal").getDeclaredField("allTypes");
        types.setAccessible(true);
        var previous = types.get(null);
        if (previous == null) types.set(null, Set.of(LightningKey.HIGH_VOLTAGE.getType()));
        try {
            long[] extracted = {0};
