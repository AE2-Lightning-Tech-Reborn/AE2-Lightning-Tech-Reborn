...
public final class TianshuSeedRefillGameTests {
    @GameTest(template = "workstation_test", timeoutTicks = 200)
    public static void reconcileThroughRealControllerAndMeNetwork(GameTestHelper helper) throws Exception {
        ...
        controller.scanNow();
        helper.assertTrue(controller.isFormed(), "native Tianshu forms: " + controller.issueText());
        ...
        helper.runAfterDelay(50, () -> {
            ...
            helper.assertTrue(controller.insertReusableSeed(seed, 32, Actionable.MODULATE) == 32, "seed stock inserted");
            helper.assertTrue(controller.insertReusableSeed(obsolete, 16, Actionable.MODULATE) == 16, "obsolete stock inserted");
            var trimmed = TianshuSeedRefillService.refillAll(port);
            helper.assertTrue(trimmed.complete(), "trim completes");
            helper.assertTrue(controller.reusableSeedAmount(seed) == 8 && controller.reusableSeedAmount(obsolete) == 0,
                    "only configured eight seed sets remain");
            helper.assertTrue(network.extract(seed, 99, Actionable.SIMULATE, source) == 24
            ...
            helper.assertTrue(deleted.complete() && controller.reusableSeedSnapshot().isEmpty(), "deleting final pattern returns all seeds");
            helper.assertTrue(network.extract(seed, 99, Actionable.SIMULATE, source) == 32, "all original seeds conserved");
            ...
            helper.assertTrue(!blocked.complete() && controller.reusableSeedAmount(obsolete) == 5,
                    "ME rejection leaves all seeds in physical storage");
            var sync = SeedRefillSync.of(blocked);
            helper.assertTrue(sync.state() == SeedRefillSync.STATE_RETURN_BLOCKED, "return failure has dedicated status");
            ...
            try {
                ...
                helper.assertTrue(decoded.equals(sync) && buffer.readableBytes() == 0, "status packet round trip preserves return key and count");
            } finally { buffer.release(); }
            drive.getInternalInventory().setItemDirect(0, savedNetworkCell);
            var retried = TianshuSeedRefillService.refillAll(port);
            helper.assertTrue(retried.complete() && controller.reusableSeedSnapshot().isEmpty(), "retry after restoring destination succeeds");
            helper.assertTrue(network.extract(obsolete, 99, Actionable.SIMULATE, source) == 21, "rejected and retried seeds are conserved");

            repository.add(payload);
            port.closedLoopPatternsChanged();
            var filled = TianshuSeedRefillService.refillAll(port);
            helper.assertTrue(filled.complete() && controller.reusableSeedAmount(seed) == 8, "ordinary refill still works after cleanup");
            helper.assertTrue(TianshuSeedRefillService.refillAll(port).moved().isEmpty(), "repeated click does not duplicate seeds");
            helper.succeed();
        });
    }
}
