...
public final class PigmeeCatalyzerJadeClientProbe {
        ...
        try {
            ...
            switch (phase) {
                case 0 -> onServer(() -> {
                    ...
                    player.teleportTo(level, .5, 100, 3, java.util.Set.of(), 180, 25, false);
                });
                case 1 -> {
                    if (!tooltipReady(POS)) return;
                    var tooltip = snownee.jade.JadeClient.tickHandler().rootElement.getTooltip();
                    require(tooltip.get(TooltipIds.GRID_NODE_STATE).isEmpty(),
                            "Pigmee still displays AE grid status: " + tooltip.toString());
                    capture("pigmee-jade-standalone.png");
                    record("Pigmee Jade: " + tooltip.toString());
                    onServer(() -> {
                        var player = mc.getSingleplayerServer().getPlayerList().getPlayer(mc.player.getUUID());
                        player.teleportTo(player.level(), 2.5, 100, 3, java.util.Set.of(), 180, 25, false);
                    });
                }
                case 2 -> {
                    ...
                    var tooltip = snownee.jade.JadeClient.tickHandler().rootElement.getTooltip();
                    require(!tooltip.get(TooltipIds.GRID_NODE_STATE).isEmpty(),
                            "normal catalyzer lost its grid status: " + tooltip.toString());
                    capture("normal-jade-grid-state.png");
                    record("Normal Jade: " + tooltip.toString());
                    open(POS);
                }
                case 3 -> {
    ...
    private static boolean tooltipReady(BlockPos pos) {
        ...
        return snownee.jade.JadeClient.tickHandler().state != null
                && snownee.jade.JadeClient.tickHandler().state.accessor() instanceof BlockAccessor accessor
                && accessor.getPosition().equals(pos)
                && snownee.jade.JadeClient.tickHandler().rootElement != null
                && !snownee.jade.JadeClient.tickHandler().getData().isEmpty();
    }
    ...
    private static void capture(String name) {
        var mc = Minecraft.getInstance();
        Screenshot.grab(mc.gameDirectory, name, mc.getMainRenderTarget(), 1, message -> record(message.getString()));
    }
