...
public final class JeiWirelessSupplyGameTests {
    ...
    private static int count(ServerPlayer p) {
        return p.getInventory().getNonEquipmentItems().stream().filter(s -> s.is(Items.OAK_PLANKS)).mapToInt(ItemStack::getCount).sum();
    }
    ...
    public static void supplyAndNativeTransfer(GameTestHelper helper) {
        ...
        helper.runAfterDelay(50, () -> {
            ...
            player.containerMenu = menu;
            // FakePlayer never receives a normal login event; initialize the native slot definitions.
            var curiosInventory = top.theillusivec4.curios.api.CuriosApi.getCuriosInventory(player).orElseThrow();
            curiosInventory.loadDatapacks();
            var curios = curiosInventory.getEquippedCurios();
            ...
            double curioEnergy = terminal.getOrDefault(AEComponents.STORED_ENERGY, 0.0);
            var curioOffer = WirelessJeiSupply.handle(player, packet(player, 9, false, materials));
            var curioTake = WirelessJeiSupply.handle(player, packet(player, 9, true, materials));
            var curioLocator = TianshuWirelessIngredientSource.locate(player).getFirst();
            var curioHost = TianshuWirelessIngredientSource.open(player, curioLocator);
            check(curioOffer.status() == 1 && curioTake.status() == 2,
                    "Curios refill: offer=" + curioOffer + ", take=" + curioTake
                    + ", host=" + curioHost + ", sameItem=" + (curioLocator.locateItem(player) == terminal));
            check(curioHost != null && curioHost.getItemStack() == terminal
                    && terminal.getOrDefault(AEComponents.STORED_ENERGY, 0.0) < curioEnergy,
                    "Curios extraction debits the actual equipped terminal, not a resource snapshot");
            var replacement = terminal.copy();
            curios.setStackInSlot(0, replacement);
            check(!curioHost.isValid(), "replacing an equipped terminal invalidates the old workstation owner");
            ...
            var before = WirelessJeiInventoryPlan.copy(player.getInventory().getNonEquipmentItems());
            var simulation = WirelessJeiInventoryPlan.insert(player.getInventory().getNonEquipmentItems(), List.of(materials));
