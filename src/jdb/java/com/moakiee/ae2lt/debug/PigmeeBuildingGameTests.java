...
public final class PigmeeBuildingGameTests {
    ...
    private static int materials(ServerPlayer player) {
        return player.getInventory().getNonEquipmentItems().stream().filter(s -> s.is(ModBlocks.PIGMEE_BUILDING_BLOCK.asItem()))
    ...
    public static void recipeLoadsIsVisibleAndSurvivesNetworkSync(GameTestHelper helper) {
        ...
        var recipe = (PigmeeBuildingRecipe) level.recipeAccess()
                .byKey(net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.RECIPE, Identifier.parse("ae2lt:pigmee_building_block"))).orElseThrow().value();
        check(recipe instanceof ShapedRecipe && !recipe.isSpecial(), "visible shaped recipe for recipe viewers");
        check(recipe.getWidth() == 3 && recipe.getHeight() == 3, "3 by 3 layout");
        check(recipe.matches(input(), level), "loaded recipe matches the cobblestone ring");
        check(!recipe.matches(CraftingInput.of(2, 2, java.util.Collections.nCopies(4, new ItemStack(Items.COBBLESTONE))), level), "player 2 by 2 grid cannot craft this recipe");
        ...
        try {
            ...
            var result = copy.assemble(input());
            check(copy.matches(input(), level), "network recipe retains ingredients and shape");
            check(result.is(ModBlocks.PIGMEE_BUILDING_BLOCK.asItem()) && result.getCount() == 64,
                    "8 cobblestone gives 64 building blocks");
            check(ItemStack.matches(copy.getRemainingItems(input()).get(4), pigmee(1)),
        ...
        check(!recipe.matches(CraftingInput.of(3, 3, wrong), level), "incomplete ring rejected");
        wrong = new ArrayList<>(input().items());
        wrong.set(4, new ItemStack(ModFumos.CREATIVE_PIGMEE_FUMO_ITEM.get()));
        check(!recipe.matches(CraftingInput.of(3, 3, wrong), level), "only ordinary Pigmee is the catalyst");
        wrong = new ArrayList<>(input().items());
        wrong.set(0, pigmee(1));
        wrong.set(4, new ItemStack(Items.COBBLESTONE));
        check(!recipe.matches(CraftingInput.of(3, 3, wrong), level), "off-center catalyst rejected");
        helper.succeed();
    }
    ...
    public static void pickupAndShiftCraftPreserveNamedStackedPigmee(GameTestHelper helper) {
        ...
        check(menu.getCarried().is(ModBlocks.PIGMEE_BUILDING_BLOCK.asItem())
                && menu.getCarried().getCount() == 64, "normal click yields a full stack");
        check(ItemStack.matches(menu.getSlot(5).getItem(), pigmee(3)), "stacked catalyst unchanged");
        for (int i = 1; i <= 9; i++) {
            if (i != 5) check(menu.getSlot(i).getItem().getCount() == 1, "one cobblestone consumed per outer slot");
        }
        player.getInventory().add(menu.getCarried());
        menu.setCarried(ItemStack.EMPTY);
        menu.clicked(0, 0, ContainerInput.QUICK_MOVE, player);
        check(materials(player) == 128, "two crafts yield exactly 128 blocks");
        check(ItemStack.matches(menu.getSlot(5).getItem(), pigmee(3)), "Pigmee count and components survive repeated crafting");
        check(menu.getSlot(0).getItem().isEmpty(), "crafting stops when cobblestone runs out");
        helper.succeed();
    }
    ...
    public static void shiftCraftStopsAtFullInventoryWithoutSpendingIngredients(GameTestHelper helper) {
        ...
        menu.clicked(0, 0, ContainerInput.QUICK_MOVE, player);
        check(materials(player) == 64, "one free slot allows exactly one craft");
        check(ItemStack.matches(menu.getSlot(5).getItem(), pigmee(1)), "full inventory does not lose Pigmee");
        for (int i = 1; i <= 9; i++) {
            if (i != 5) check(menu.getSlot(i).getItem().getCount() == 2, "backpressure must not spend extra cobblestone");
        }
        menu.clicked(0, 0, ContainerInput.QUICK_MOVE, player);
        check(materials(player) == 64 && menu.getSlot(1).getItem().getCount() == 2, "blocked retry is inert");
        helper.succeed();
    }

    public static void otherPigmeeRecipesStillConsumePigmee(GameTestHelper helper) {
        var recipe = (ShapedRecipe) helper.getLevel().recipeAccess()
                .byKey(net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.RECIPE, Identifier.parse("ae2lt:pigmee_synthesis_station"))).orElseThrow().value();
        var items = recipe.getIngredients().stream()
                .map(ingredient -> ingredient.isEmpty() ? ItemStack.EMPTY : ingredient.orElseThrow().items().map(ItemStack::new).toArray(ItemStack[]::new)[0].copy()).toList();
        var input = CraftingInput.of(recipe.getWidth(), recipe.getHeight(), items);
        check(recipe.matches(input, helper.getLevel()), "existing station fixture matches");
        var remaining = recipe.getRemainingItems(input);
        for (int i = 0; i < input.size(); i++) {
            if (input.getItem(i).is(ModFumos.PIGMEE_FUMO_ITEM.get())) {
                check(remaining.get(i).isEmpty(), "existing recipes still consume ordinary Pigmee");
            }
        }
        helper.succeed();
    }
    ...
    public static void basicBlockHasSimplePlacementAndSelfDrop(GameTestHelper helper) {
        ...
        check(!state.hasBlockEntity(), "floor must not create a block entity");
        var tool = new ItemStack(Items.WOODEN_PICKAXE);
        check(tool.isCorrectToolForDrops(state), "wooden pickaxe is sufficient");
        var drops = Block.getDrops(state, helper.getLevel(), helper.absolutePos(TABLE), null, player, tool);
        check(drops.size() == 1 && drops.getFirst().is(block.asItem()) && drops.getFirst().getCount() == 1,
    ...
    public static void allThirtyTwoPanelsPlaceAndDropWithoutBlockEntities(GameTestHelper helper) {
        check(ModBlocks.PIGMEE_BUILDING_PANELS.size() == 16
        ...
        for (var panels : java.util.List.of(ModBlocks.PIGMEE_BUILDING_PANELS, ModBlocks.PIGMEE_FRAMED_BUILDING_PANELS)) {
            for (DyeColor color : DyeColor.values()) {
                ...
                check(ids.add(block), "each style/color has a distinct block identity");
                check(block.color() == color, "registered color matches map key");
                helper.setBlock(TABLE, block);
                var state = helper.getLevel().getBlockState(helper.absolutePos(TABLE));
                check(!state.hasBlockEntity(), "decorative panels have no block entities");
                check(tool.isCorrectToolForDrops(state), "all panels are mineable with a wooden pickaxe");
                var drops = Block.getDrops(state, helper.getLevel(), helper.absolutePos(TABLE), null, player, tool);
                check(drops.size() == 1 && drops.getFirst().is(block.asItem())
    ...
    public static void allInputsOfferAllThirtyTwoDyeFreeStonecuttingOutputs(GameTestHelper helper) {
        ...
        for (int inputIndex = 0; inputIndex < inputs.size(); inputIndex++) {
            ...
            check(menu.getVisibleRecipes().entries().size() == 32, "every basic/finished input offers all 32 choices");
            var actual = new java.util.HashSet<net.minecraft.world.item.Item>();
            for (var recipe : menu.getVisibleRecipes().entries()) {
                var result = recipe.recipe().optionDisplay().resolveForFirstStack(net.minecraft.world.item.crafting.display.SlotDisplayContext.fromLevel(helper.getLevel()));
                check(result.getCount() == 1, "every conversion is strictly 1:1");
                actual.add(result.getItem());
            }
            check(actual.equals(expected), "all 16 colors and both styles are selectable without dye");
    ...
    public static void finishedPanelsReturnOneBasicBlockInBothCraftingGrids(GameTestHelper helper) {
        ...
        for (var block : panels()) {
            ...
            table.clicked(0, 0, ContainerInput.PICKUP, player);
            check(table.getCarried().is(ModBlocks.PIGMEE_BUILDING_BLOCK.asItem())
            ...
            inventory.clicked(0, 0, ContainerInput.PICKUP, player);
            check(inventory.getCarried().is(ModBlocks.PIGMEE_BUILDING_BLOCK.asItem())
    ...
    public static void conversionRecipesDoNotAcceptUnrelatedMaterials(GameTestHelper helper) {
        ...
        var restoration = (net.minecraft.world.item.crafting.ShapelessRecipe) helper.getLevel().recipeAccess()
                .byKey(net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.RECIPE, Identifier.parse("ae2lt:pigmee_building_block_from_panels"))).orElseThrow().value();
        for (var item : java.util.List.of(Items.COBBLESTONE, Items.STONE, Items.WHITE_CONCRETE, Items.RED_DYE,
            ...
            check(!restoration.matches(CraftingInput.of(1, 1, java.util.List.of(new ItemStack(item))),
            ...
            check(menu.getVisibleRecipes().entries().stream().noneMatch(recipe -> panelItems.contains(
                    recipe.recipe().optionDisplay().resolveForFirstStack(net.minecraft.world.item.crafting.display.SlotDisplayContext.fromLevel(helper.getLevel())).getItem())),
        ...
        check(!restoration.matches(CraftingInput.of(2, 1, twoPanels), helper.getLevel()),
    ...
    private static void cut(GameTestHelper helper, ServerPlayer player, Block input, Block expected) {
        ...
        int selected = -1;
        for (int i = 0; i < menu.getVisibleRecipes().entries().size(); i++) {
            if (menu.getVisibleRecipes().entries().get(i).recipe().optionDisplay().resolveForFirstStack(net.minecraft.world.item.crafting.display.SlotDisplayContext.fromLevel(helper.getLevel())).is(expected.asItem())) {
                selected = i;
                break;
            }
        }
        check(selected >= 0 && menu.clickMenuButton(player, selected), "requested style is offered by the real stonecutter");
        check(menu.getSlot(1).getItem().is(expected.asItem()) && menu.getSlot(1).getItem().getCount() == 1,
                "cutting offers exactly 1 output of the expected color/style");
        menu.clicked(1, 0, ContainerInput.PICKUP, player);
        check(menu.getSlot(0).getItem().getCount() == 2 && menu.getCarried().is(expected.asItem())
