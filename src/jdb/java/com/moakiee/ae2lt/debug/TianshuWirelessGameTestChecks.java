...
final class TianshuWirelessGameTestChecks {
    ...
    static void run(GameTestHelper helper) {
        ...
        manualInputs.add(com.moakiee.ae2lt.recipe.compat.LegacyItemStackNbt.save(new ItemStack(Items.NETHERITE_INGOT, 2), helper.getLevel().registryAccess()));
        ...
        occupiedSource.set(manualComponent, net.minecraft.world.item.component.CustomData.of(manualData));
        var emptyTarget = new ItemStack(AE2wtlibItems.UNIVERSAL_TERMINAL);
        emptyTarget.set(WTDefinitions.CRAFTING.componentType(), Unit.INSTANCE);
        var manualMerged = Common.mergeTerminal(emptyTarget, occupiedSource, definition);
        require(!manualMerged.isEmpty() && manualMerged.get(manualComponent).copyTag().equals(manualData),
                "native WUT merge retains manual workstation contents");
        var occupiedTarget = emptyTarget.copy();
        occupiedTarget.set(manualComponent, net.minecraft.world.item.component.CustomData.of(manualData));
        ...
        require(occupiedSource.get(manualComponent).copyTag().equals(manualData)
                        && occupiedTarget.get(manualComponent).copyTag().equals(manualData),
        ...
        require(AE2wtlibItems.UNIVERSAL_TERMINAL.asItem().getUpgrades(target).addItems(new ItemStack(AE2wtlibItems.MAGNET_CARD)).isEmpty(), "WUT magnet upgrade registration");
        ...
        try {
            ...
            menu.clicked(helmet.index, 0, ContainerInput.PICKUP, player);
            ...
            menu.clicked(upgrades.getFirst().index, 0, ContainerInput.PICKUP, player);
            ...
            menu.clicked(upgrades.get(7).index, 0, ContainerInput.PICKUP, player);
            ...
            menu.receiveClientAction("togglepickupmode", new byte[0]);
            ...
            menu.clicked(menu.getSlots(Ae2ltSlotSemantics.TIANSHU_ANVIL).getFirst().index, 0, ContainerInput.PICKUP, player);
