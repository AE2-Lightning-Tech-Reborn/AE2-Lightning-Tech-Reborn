...
public final class PigmeeBuildingClientProbe {
    ...
    private static void buildFixture() {
        ...
        level.getGameRules().set(GameRules.SPAWN_MOBS, false, mc.getSingleplayerServer());
        ...
        player.teleportTo(level, 38.5, 175, 38.5, Set.of(), 180, 90, false);
        fixtureReady = true;
    }
    ...
    private static void checkModels() {
        ...
        for (var panels : java.util.List.of(ModBlocks.PIGMEE_BUILDING_PANELS, ModBlocks.PIGMEE_FRAMED_BUILDING_PANELS)) {
            for (var color : DyeColor.values()) {
                ...
                var model = mc.getModelManager().getBlockStateModelSet().get(block.defaultBlockState());
                require(model instanceof ConnectedTextureBakedModel, "CTM loader for " + block);
                var name = model.particleMaterial().sprite().contents().name().toString();
                ...
                mc.getItemModelResolver().updateForTopItem(item, new ItemStack(block),
                        net.minecraft.world.item.ItemDisplayContext.GUI, mc.level, mc.player, 0);
                require(!item.isEmpty(), "item layers " + block);
                require(!item.pickParticleMaterial(RandomSource.create(0)).sprite().contents().name()
                        .toString().contains("missingno"), "item sprite " + block);
                var bounds = item.getModelBoundingBox();
                require(bounds.getXsize() > 0 && bounds.getYsize() > 0 && bounds.getZsize() > 0,
                        "native cube GUI model " + block);
            }
        }
    }
    ...
    private static void checkConnections() throws Exception {
        ...
        for (Direction face : Direction.values()) for (int kind = 0; kind < 4; kind++) {
            ...
            var model = (ConnectedTextureBakedModel) mc.getModelManager().getBlockStateModelSet().get(state);
            var connection = connections(model, pos, state);
            ...
            model.collectParts(mc.level, pos, state, RandomSource.create(0), parts);
            ...
            for (int sq = 0; sq < 2; sq++) for (int tq = 0; tq < 2; tq++) {
                ...
                require(Math.abs(UVPair.unpackU(quad.packedUV(0)) - u) < .00001
                                && Math.abs(UVPair.unpackV(quad.packedUV(0)) - v) < .00001,
        ...
        var state = mc.level.getBlockState(pos);
        var model = (ConnectedTextureBakedModel) mc.getModelManager().getBlockStateModelSet().get(state);
        require(connections(model, pos, state).edges(Direction.UP) == 0,
                "different colors and different styles must remain disconnected");
    }

    private static CtmConnectionState connections(ConnectedTextureBakedModel model, BlockPos pos,
            net.minecraft.world.level.block.state.BlockState state) throws Exception {
        var method = ConnectedTextureBakedModel.class.getDeclaredMethod("connectionState",
                net.minecraft.client.renderer.block.BlockAndTintGetter.class, BlockPos.class,
                net.minecraft.world.level.block.state.BlockState.class);
        method.setAccessible(true);
        return (CtmConnectionState) method.invoke(model, Minecraft.getInstance().level, pos, state);
    }
    ...
    private static void capture(String name) {
        var mc = Minecraft.getInstance();
        Screenshot.grab(mc.gameDirectory, name, mc.getMainRenderTarget(), 1, text -> {});
    }
