...
ECO、Data 与天枢分别维护自己的规划器、规划界面和执行器；模组是否存在不会自动改变其他系统的主控权。只有 ECO 主控时由 ECO 全权执行，只有 Data 或天枢时分别由对应系统全权执行。

跨模组只通过窄的、可选的 API 能力边界：ECO 可以把适合的步骤提交给 Lightning 的过载样板供应器进行自适应批量/翻倍；天枢 CPU 可以选择使用 ECO fastpath/F 系列主机；ECO CPU 也可以把步骤提交给天枢合成器。规划模型、规划界面、执行器、CPU、provider 和内部 job 状态不跨模组共享。提供方缺失、版本不兼容或拒绝请求时，调用方回退到自己的执行器。

完整接口草案与线程、幂等、回退约束见 `docs/independent-planners-and-cross-mod-capabilities.md`。
